...
新会话开始时，先阅读本节、`docs/roadmap.md` 和 [`docs/prd/document-tag-and-association-rag-prd.md`](./document-tag-and-association-rag-prd.md)，然后按以下顺序推进：桌面 Web 的 19.1.3 第五项当前能力验收已完成，下一优先级是关系类型白名单与跨分片实体合并评估；二阶段自然全文摘要的 Fake/SQLite/MockMvc 验收已完成，真实模型质量边界仍需单独补充。之后执行专项阶段 0，准备固定正负样本并冻结文档关系图、节点详情跳转、悬浮高亮和默认内容关联规则；再按“无 Embedding 文档内容关联 → 可选标签 → 文档关系图切换 → Embedding/混合 RAG → 固定 RAG 有据问答 → 条件满足后 Agent”推进。移动端、触控和读屏验收顺延到后续专项；当前不提前引入标签、文档关系、会话引用、Embedding 或 Agent 生产代码，也不重新搭建已完成的前后端脚手架、PDF、分页、SQLite 连接池、SSE 流式抽取和三 Tab 来源资料链路。
...
以下事项是 AI 抽取链路进入可用状态前的配套任务；第 1、2、3、4、5、6、8 项和候选审核持久化已完成当前桌面 Web 代码与验证边界，下一优先级进入第 7 项审核质量切片：
...
5. **当前验证顺序**：
   ...
   - 下一优先完成关系类型白名单与跨分片实体合并评估；
   - 在具备真实模型密钥和受控预算时，单独补充二阶段自然全文摘要的单/多分片质量边界，不以 Fake 结果代替真实模型结论；
