...
- 已使用 Java 21 执行根 Reactor `mvn test`，共 39 项测试通过；除分页、AI 和 PDF 既有回归外，新增覆盖 Hikari 池上限与获取超时、每连接 WAL/外键/忙等待、WAL 读写边界、第二写者 `SQLITE_BUSY`，以及池上限为 1 时 Fake 模型阻塞期间仍可导入和查看资料；真实模型和真实 Embedding 调用未纳入默认自动测试。
...
新会话开始时，先阅读本节和 `docs/roadmap.md`，然后从 19.1.3 的第二项继续，不重新搭建前后端脚手架，也不重做文本型 PDF、分页和 SQLite 连接池链路。
...
以下事项是 AI 抽取链路进入可用状态前的配套任务；第 1 项已完成验收，其余仍是待实现设计：
