...
- 已确认并完成首个前端小切片：后续默认主线不再展示独立“图谱类型”导航或筛选；图谱类型不再作为新增文档分析目标，左侧已改为“待处理”状态入口和标签空态。现有类型字段及历史实体图谱仅保留兼容读取，真实标签数据和标签筛选仍待专项阶段 2。
...
- 已使用 Java 21 执行根 Reactor `mvn test`，共 51 项测试通过；除分页、AI、PDF 和 SQLite 既有回归外，新增覆盖 SSE 多分片事件顺序、真实 Fake 增量转发、结构校验失败、失败运行恢复、OpenAPI 流媒体类型、OpenAI-compatible 流式完整文本解析和证据校验，以及单/多分片文档级全文摘要成功、摘要失败回退边界；真实流式模型、真实全文摘要质量和真实 Embedding 调用未纳入默认自动测试。
...
- 结构化输出和 Prompt 已升级为 `prd-extraction-v2` / `extraction-v2`：每个分片必须返回只基于当前原文的非空短摘要，服务端使用 Jakarta Validation 校验非空和 160 字边界；分片完成后再通过独立的 `document-summary-v1` Prompt 发起一次 Reduce 调用，按章节顺序输入分片摘要并生成自然全文摘要，不再使用分号拼接。
...
- 已确认文档关联主线的产品边界：默认业务节点收敛为真实来源文档，默认先按文档内容关联；标签仅作为用户可选的筛选、补充候选和解释条件。独立图谱类型不再是后续用户流程或新增 AI 识别的必选维度。当前尚未实现标签表、文档-标签关系、文档关系表或文档关系图；左侧固定图谱类型前端区域已移除，真实标签导航仍待专项阶段 2。
...
新会话开始时，先阅读本节和 `docs/roadmap.md`，然后按以下顺序推进：先完成 19.1.3 第五项的浏览器与流式交互验证，再进行关系类型白名单与跨分片实体合并评估；二阶段自然全文摘要的 Fake/SQLite/MockMvc 验收已完成，下一步只需补充真实模型质量边界。完成浏览器、关系质量和实体合并的最小验收后，进入文档标签与文档关联专项的阶段 0 评审和固定资料集准备。左侧信息架构改造可作为前端独立小切片提前实施，但不得提前引入未评审的标签/关联数据接口。不要重新搭建前后端脚手架，也不重做文本型 PDF、分页、SQLite 连接池、SSE 流式抽取和三 Tab 来源资料弹窗链路。
...
以下事项是 AI 抽取链路进入可用状态前的配套任务；第 1、2、3、4、6、8 项和候选审核持久化已完成当前代码与自动验证边界，第 5 项仍需真实浏览器与流式网络验证，后续再推进第 7 项审核质量切片：
...
4. **左侧信息架构**：已将“图谱类型”区域替换为“待处理 + 标签空态”；待标签接口落地前只显示现有状态数据或明确空态，不伪造标签数量。当前不恢复图谱类型筛选；如果未来兼容旧实体图谱确有需要，必须作为局部工具栏单独评估。
5. **当前验证顺序**：
   - 先完成真实浏览器下的来源资料无限滚动、卡片多选、批量操作、三 Tab、空态、历史恢复、错误/重试、连接中断和移动端/读屏验证；
   - 再验证已实现的二阶段自然全文摘要，覆盖单/多分片、汇总失败回退和真实模型质量边界；
   - 再完成关系类型白名单与跨分片实体合并评估；
   - 之后评审并冻结“默认内容关联、标签可选、RAG 后置”等专项产品决策，准备固定正负样本；
   - 最后按“非向量内容候选召回 → 文档关系判断 → 证据校验 → 关联审核 → 可选标签生成/审核/关联 → 文档关系图 → RAG/Embedding 评估”的顺序实现。
6. **禁止提前扩张**：在上述前置验证和专项阶段 0 未完成前，不新增标签表、文档关系表、Embedding 索引或 Agent 编排，也不把文档关系图直接替换为默认图谱页面。
...
4. **已确认产品方向，待进入阶段 0**：默认主线为文档内容关联，标签关联可选；不再保留独立图谱类型导航或把实体类型识别作为新增主线前置；现有实体图谱保留兼容和实验用途。先评审 [`docs/prd/document-tag-and-association-rag-prd.md`](./document-tag-and-association-rag-prd.md)，冻结内容关联优先级、标签审核、五种文档关系白名单、文档关系图主视图和手工关系。
5. **专项方案实施顺序**：先完成固定正负样本，再实现无 Embedding 的显式引用/标题/摘要/关键词内容候选关联、关系判断和审核；随后实现可选标签生成、审核和标签候选补充；最后用独立实验评估是否引入向量召回和混合 RAG。未达到评估门槛前不把向量相似度当作正式关系依据。标签、文档关系和关联审核接口均须在阶段 0 评审后设计。
