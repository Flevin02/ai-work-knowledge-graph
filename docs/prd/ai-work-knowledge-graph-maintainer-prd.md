...
- 当前后端只解析 UTF-8 Markdown/TXT；PDFBox 依赖已经进入 Maven 管理，但文本型 PDF 解析器尚未实现，并已明确列为下一任务。该任务首版不包含扫描件 OCR；DOCX 仍留在后续主线。
...
## 19.1 下一优先级：文本型 PDF 与 AI 抽取主线

### 19.1.1 最高优先级：文本型 PDF 来源资料导入

下一会话先完成可复制文本 PDF 的最小导入闭环，不同时实现 DOCX、OCR、版面还原或 PDF 表格结构化：
...
5. **验收边界**：Java 21 根 Reactor 测试、前端类型检查和生产构建必须通过，并至少完成一次隔离本地 HTTP/浏览器导入预览；不得把文本抽取成功表述为表格结构、图片、阅读顺序或 OCR 已正确恢复。
