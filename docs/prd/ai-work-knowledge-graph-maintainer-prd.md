...
- 页面可以从服务端恢复 `processing` 状态，但尚未实现只对当前页处理中记录进行轻量刷新；如果用户在另一个页面或会话启动抽取，当前页面不会自动感知其完成或失败。
...
3. **补充可观测验证**：当前实现从结构上固定为 MyBatis-Plus 的 count/分页查询加一条当前页抽取摘要窗口查询，尚未通过 SQL 计数器单独断言查询次数；后续若引入查询观测工具，应补充单页 1 条与 12 条资料的查询次数对比，防止回归为 N+1。
