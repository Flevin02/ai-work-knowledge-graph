...
- 已实现关系审核页面，可在前端状态中采纳或拒绝待审核关联。
- 已统一关系审核文案为“待审核”“采纳关联”和“已采纳”，并为审核空状态补齐与提示框一致的顶部间距。
...
- 前端来源资料卡片已增加“查看”按钮，默认使用 Markdown 适配渲染，同时支持切换“原文预览”查看 Markdown/TXT 原始文本；弹窗标题会随“渲染预览/原文预览”Tab 联动，支持加载、错误、Escape、遮罩关闭和移动端长文滚动，渲染默认不执行原文 HTML。
...
- 当前 AI 抽取仍以同步 HTTP 响应返回完整结果；前端虽已有独立的 AI 提取预览窗口，但来源资料查看弹窗还只有“渲染预览”和“原文预览”两个 tab，尚未在同一处展示可增量更新的 AI 输出，也没有取消、断线重连、失败后继续查看部分输出等能力。
- 当前来源资料分页、总数和抽取摘要已经通过 MockMvc/SQLite 集成测试与前端生产构建，但尚未执行浏览器点击分页、悬浮提示、切换空间和删除页末项的自动化回归。
- 页面已实现只对当前页服务端 `processing` 记录进行轻量刷新，并通过 TypeScript 检查和生产构建；尚未在浏览器中验证跨页面或跨会话启动抽取后的状态自动收敛、轮询失败重试以及切换空间/页码时取消在途请求。
...
v0.12 已完成分页接口、MyBatis-Plus SQLite 分页插件、最近运行与最近成功结果分离、当前页批量抽取状态、最近成功 AI 摘要替换列表预览、首屏按钮状态、前端页码交互、无结果提示、处理中状态轻量刷新和 OpenAPI/TypeScript 契约同步。下一会话不要重做这些能力，只处理以下剩余验收项：

1. **浏览器交互回归**：在真实前后端本地服务上验证多页翻页、切换空间、导入后第一页刷新、删除第二页最后一条、无结果悬浮提示、最近失败但存在历史成功结果时的“查看上次结果”，并补充 `processing` 状态自动收敛及切换空间/页码停止刷新。当前只有自动测试、类型检查和生产构建证据。
2. **补充可观测验证**：当前实现从结构上固定为 MyBatis-Plus 的 count/分页查询加一条当前页抽取摘要窗口查询，尚未通过 SQL 计数器单独断言查询次数；后续若引入查询观测工具，应补充单页 1 条与 12 条资料的查询次数对比，防止回归为 N+1。
