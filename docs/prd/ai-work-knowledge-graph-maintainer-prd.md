...
新会话开始时，先阅读本节和 `docs/roadmap.md`，然后从 19.1.3 的第四项继续，不重新搭建前后端脚手架，也不重做文本型 PDF、分页、SQLite 连接池、SSE 流式抽取和三 Tab 来源资料弹窗链路。
...
以下事项是 AI 抽取链路进入可用状态前的配套任务；第 1、2、3 项和候选审核持久化已完成当前代码与自动验证边界，第 4 项仍未完成，第 5 项只完成后端/Fake AI 与部分前端状态骨架，第 6 项名称搜索已完成，后续应先收口第 4、5 项，再推进第 7 项审核质量切片：
