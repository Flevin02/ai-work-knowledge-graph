...
以下事项是 AI 抽取链路进入可用状态前的配套任务；第 1、2 项和候选审核持久化已完成当前自动验证，第 3 项是弹窗体验收口，第 6 项名称搜索已完成，下一步按第 7 项继续推进：
...
7. **后续审核质量切片：关系类型白名单与跨分片实体合并评估**：先为关系类型建立允许集合和主体/客体类型约束，拒绝模型输出的未知或语义不匹配关系；再使用固定多分片资料评估同一实体在不同分片中的规范化、来源合并和关系去重。两项都必须保留失败样例，不能因为 JSON 合法或证据可反查就直接确认业务语义正确。
