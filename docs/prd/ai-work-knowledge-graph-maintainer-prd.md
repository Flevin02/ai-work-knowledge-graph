...
## 2.4 文档关联主线调整（方向已确认，专项方案待实施）

当前实现已经完成实体、关系、证据和审核的技术链路，但来源资料页面的主体验需要从“平铺实体关系”收敛为“文档标签与文档关联建议”。专项产品与技术设计见 [`docs/prd/document-tag-and-association-rag-prd.md`](./document-tag-and-association-rag-prd.md)，该方案尚未进入实现状态，不能把其中的表、接口、指标或 RAG 链路描述为已完成能力。
...
- 已确认并完成首个前端小切片：后续默认主线不再展示独立“图谱类型”导航或筛选；图谱类型不再作为新增文档分析目标，左侧已改为“待处理”状态入口和标签空态。现有类型字段及历史实体图谱仅保留兼容读取；后端真实标签数据与统计接口已具备，桌面 Web 标签导航和筛选仍待下一联调切片。
...
- 已确认文档关联主线的产品边界：默认业务节点收敛为真实来源文档，默认先按文档内容关联；标签仅作为用户可选的筛选、补充候选和解释条件。独立图谱类型不再是后续用户流程或新增 AI 识别的必选维度。文档关系持久化和后端 API 已落地；标签字典、文档标签、标签证据、独立 Fake 标签运行、标签查询、不可变审核历史和批量审核 API 已完成，但真实模型、前端标签运行/导航/审核联调和文档关系图仍待专项阶段 2/4。
...
新会话开始时，先阅读本节、`docs/roadmap.md`、[`docs/prd/document-tag-and-association-rag-prd.md`](./document-tag-and-association-rag-prd.md)、[`docs/tests/document-association-evaluation-report-v1.md`](../tests/document-association-evaluation-report-v1.md) 和 `document-tag-v1` 契约，然后直接进入专项阶段 2 的桌面 Web 联调小切片：接入标签运行创建/恢复、文档标签查询、空间已确认标签统计和批量 `accept/reject` 审核 API，在来源资料 AI 概览与左侧标签区展示真实服务端状态，并覆盖空态、处理中、成功、失败、重复审核冲突和刷新恢复。当前只做桌面 Web，不同时实现移动端、真实模型、标签候选补充、Embedding、文档关系图、问答或 Agent。
...
以下事项是 AI 抽取链路进入可用状态前的配套任务；第 1～13 项、候选审核持久化、无 Embedding 候选召回、Fake 关系判断、逐字证据校验、后端审核 API、固定资料完整指标评估、标签持久化基础、Fake 标签运行和标签审核后端闭环均已完成各自记录的代码和验证边界，下一优先级进入专项阶段 2 的桌面 Web 标签联调：
...
13. **已完成当前自动验证：标签查询与不可变审核闭环**：新增文档级标签查询、批量采纳/拒绝和空间级已确认标签统计 API；响应批量组装标签定义、证据与审核历史，空间统计通过 MyBatis XML Join 只计算 `confirmed` 标签和有效来源资料。审核请求只接受服务端文档标签关系标识、动作和可选原因，先验证同批重复、空间/文档归属和 `suggested` 当前态，再在同一事务中更新状态并插入唯一审核历史；跨文档批次、重复标识或重复审核不会产生部分结果。新增 3 项测试后 Java 21 根 Reactor 全量 83 项测试通过；桌面 Web、浏览器、真实标签模型、固定资料标签 Precision/Recall 和生产未验证。
...
4. **左侧信息架构**：已将“图谱类型”区域替换为“待处理 + 标签空态”；后端空间已确认标签统计接口已经落地，下一桌面 Web 切片必须消费真实统计替换空态，不伪造标签数量。当前不恢复图谱类型筛选；如果未来兼容旧实体图谱确有需要，必须作为局部工具栏单独评估。
5. **当前验证顺序**：
   ...
   - 已完成阶段 1 的文档关联运行、关系、证据和审核持久化基础、无 Embedding 候选召回、Fake 关系判断、候选集合/逐字证据校验、关联审核 API 和固定资料完整关系指标评估；阶段 2 已完成标签持久化基础、独立 Fake 标签运行、Schema/业务引用/逐字证据校验、suggested 幂等物化、运行恢复、标签查询、不可变审核历史和批量审核 API，下一步直接进入桌面 Web 标签联调；
...
5. **专项方案实施顺序**：可选标签持久化、独立 Fake 标签运行、Schema/业务/证据校验、suggested 幂等物化、运行恢复、标签查询、不可变审核历史和批量审核 API 已经完成；下一小切片立即进行桌面 Web 标签运行/查询/审核联调，再推进标签候选补充及独立文档关系图前端，最后评估是否引入向量召回和混合 RAG。固定 Fake 关系基线已经达标，但 Precision@8 0.1707 仍作为后续降噪对照，不把向量相似度当作正式关系依据。
