...
新会话开始时，先阅读本节、`docs/roadmap.md` 和 [`docs/prd/document-tag-and-association-rag-prd.md`](./document-tag-and-association-rag-prd.md)，然后执行专项阶段 0：不写产品代码，先准备固定文档关联正负样本和标注答案，并冻结默认内容关联、可选标签、五种文档关系、文档关系图、节点详情跳转、悬浮高亮、Prompt/Schema/策略版本及验收规则；再按“无 Embedding 文档内容关联 → 可选标签 → 文档关系图切换 → Embedding/混合 RAG → 固定 RAG 有据问答 → 条件满足后 Agent”推进。现有实体关系白名单与跨分片精确规范化键合并已经完成 Fake/SQLite 验收；二阶段自然全文摘要和 `prd-extraction-v3` 的真实模型质量边界仍需单独补充，但不阻塞阶段 0。移动端、触控和读屏验收顺延到后续专项；当前不提前引入标签、文档关系、会话引用、Embedding 或 Agent 生产代码，也不重新搭建已完成的前后端脚手架、PDF、分页、SQLite 连接池、SSE 流式抽取和三 Tab 来源资料链路。
...
以下事项是 AI 抽取链路进入可用状态前的配套任务；第 1～8 项和候选审核持久化均已完成各自记录的代码与验证边界，下一优先级进入专项阶段 0：
...
5. **当前验证顺序**：
   ...
   - 下一优先执行专项阶段 0，评审并冻结“默认内容关联、标签可选、RAG 后置”等产品决策，准备固定正负样本和标注答案；
   - 在具备真实模型密钥和受控预算时，单独补充二阶段自然全文摘要与 `prd-extraction-v3` 关系约束的真实模型质量边界，不以 Fake 结果代替真实模型结论，也不阻塞阶段 0；
