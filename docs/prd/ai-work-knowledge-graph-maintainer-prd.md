...
- 已为文档关系图增加 `/spaces/{spaceId}/documents/{documentId}` 可恢复详情 URL：图内打开文档或关系证据时写入真实路由，直接访问可恢复知识空间、文档图模式、搜索词、选中节点及有效证据定位；关闭详情返回带状态参数的文档关系图。当前尚未建立筛选状态和 Cytoscape 视口快照，因此不把本切片描述为完整视口恢复。
...
- 已确认文档关联主线的产品边界：默认业务节点收敛为真实来源文档，默认先按文档内容关联；标签仅作为用户可选的筛选、补充候选和解释条件。独立图谱类型不再是后续用户流程或新增 AI 识别的必选维度。文档关系持久化和后端 API、标签字典、文档标签、标签证据、独立 Fake 标签运行、标签查询、不可变审核历史、批量审核 API、桌面 Web 标签运行/导航统计/审核联调、confirmed 标签候选补充、文档关系图查询、详情侧栏、关系证据定位和可恢复详情 URL 最小切片均已完成；真实标签模型、手工标签编辑、图谱筛选、视口恢复、邻居高亮和移动端仍待专项后续阶段。
...
- 文档关联阶段 1 已完成运行、关系、证据、审核的本地持久化基础、无 Embedding 候选召回、Fake 关系判断、逐字证据校验、后端审核 API 和固定资料完整指标评估。文档关系图已具备独立 `/v1/spaces/{spaceId}/document-graph` 查询、桌面切换、节点详情侧栏、边证据批量查询和可恢复详情路由；关系证据可通过 `evidenceId` 恢复到来源资料原文 Tab，并在可反查 quote 上高亮。本轮前端 typecheck/build、隔离动态路由 HTTP 200 和含证据的文档图 API 已通过；由于当前会话没有可调用的浏览器或本机 UI 控制运行时，尚未执行点击、刷新、关闭返回的可见浏览器回归。图谱筛选、Cytoscape 视口恢复、邻居高亮、真实模型关系质量和生产验证仍未完成，Precision@8 0.1707 也表明候选上下文噪声仍需后续对照优化。
...
- 2026-08-21 已完成目标态核验：当时页面仍是实体/关系混合图，节点点击只更新右侧详情，来源资料名称没有详情页跳转；标签区是空态；没有问答、引用或 Agent 运行入口。2026-08-25 已补文档关系图查询、confirmed 边过滤、真实来源文档节点、桌面切换、详情侧栏和关系证据定位；2026-08-26 已补真实动态详情 URL 及空间、搜索词、选中节点和证据定位的基础返回状态。当前仍未恢复 Cytoscape 视口，也未做图谱筛选、悬浮/键盘高亮、固定 RAG 有据问答和可选 Agent。
...
新会话开始时，先阅读本节、`docs/roadmap.md`、[`docs/prd/document-tag-and-association-rag-prd.md`](./document-tag-and-association-rag-prd.md)、[`docs/tests/document-association-evaluation-report-v1.md`](../tests/document-association-evaluation-report-v1.md)、[`docs/tests/document-association-tag-augmentation-evaluation-v1.md`](../tests/document-association-tag-augmentation-evaluation-v1.md)、[`docs/tests/document-association-tag-threshold-evaluation-v1.md`](../tests/document-association-tag-threshold-evaluation-v1.md)、[`docs/tests/document-association-tag-threshold-evaluation-v2.md`](../tests/document-association-tag-threshold-evaluation-v2.md) 和 `document-tag-v1` 契约。第一优先级已完成：文档关系图详情侧栏、关系证据定位，以及 `/spaces/{spaceId}/documents/{documentId}` 可恢复详情 URL 与基础返回状态。下一项按顺序进入文档类型/关系类型筛选最小切片，再实现悬浮/键盘一跳邻居高亮；当前不同时实现真实标签模型、Embedding、问答或 Agent，移动端、触控和读屏仍顺延。
...
5. **当前验证顺序**：
   ...
   - 已完成阶段 1 的文档关联运行、关系、证据和审核持久化基础、无 Embedding 候选召回、Fake 关系判断、候选集合/逐字证据校验、关联审核 API 和固定资料完整关系指标评估；阶段 2 已完成标签持久化基础、独立 Fake 标签运行、Schema/业务/证据校验、suggested 幂等物化、运行恢复、标签查询、不可变审核历史、批量审核 API、桌面 Web 标签联调和 confirmed 标签显式补充候选后端/前端契约；文档关系图已完成查询、confirmed 边过滤、真实来源文档节点、桌面切换、空态、节点详情侧栏、关系证据定位和可恢复详情 URL/基础返回状态，下一步进入文档类型/关系类型筛选，再实现悬浮/键盘邻居高亮；
...
5. **专项方案实施顺序**：可选标签持久化、独立 Fake 标签运行、Schema/业务/证据校验、suggested 幂等物化、运行恢复、标签查询、不可变审核历史、批量审核 API、桌面 Web 标签联调、confirmed 标签显式补充候选通道、固定资料质量对照、入口浏览器回归、独立文档关系图、详情/证据定位及可恢复详情 URL/基础返回状态已经完成；下一步依次实现文档类型/关系类型筛选与悬浮/键盘邻居高亮，最后评估是否引入向量召回和混合 RAG。固定 Fake 关系基线已经达标，但 Precision@8 0.1707 仍作为后续降噪对照，不把共同标签或向量相似度当作正式关系依据。
