...
专项方案的核心边界是：文档关系图的节点直接来自真实 `source_documents`；标签用于组织、筛选和候选召回；AI 只生成候选标签和候选文档关系；关系必须通过服务端证据校验和人工审核。现有实体图谱保留用于兼容和后续实验，不在未评审前直接删除或替换历史数据。专项设计同时预留未来 Agent 编排接口，但当前默认仍是可解释的固定 Pipeline；Agent 不能绕过知识空间隔离、候选集合、证据校验和人工审核。
...
- 多人协作和审核记录。
- 关系版本与时间旅行查询。
- 文档标签与文档关联：专项 PRD 已完成待评审设计，支持人工标签、AI 建议标签、按标签筛选、RAG 元数据过滤和文档-文档候选关联；共同标签不能直接证明正式图谱关系，仍需原文证据或人工确认。当前尚未实现。
- 会议纪要自动转任务和提醒。
- 项目周报、交接材料和活动通知自动生成。
- 图谱驱动的活动海报和多渠道物料生成。
- 企业知识域模板和可配置关系类型。
...
- Java 21 根 Reactor 全量 `mvn test` 共 47 项通过；新增覆盖批量删除软删除语义、批量提取后端线程池 2 并发和独立运行落库、批量 OpenAPI 契约，以及实体空摘要被接受、超过 160 字符被结构校验拒绝。前端 `npm run typecheck` 和 `npm run build` 通过。

## 18.2 当前验证边界

- 前端图谱、来源资料和关系审核均已具备 Java 后端读取/写入链路；没有真实图谱数据的空知识空间仍保留虚构演示图谱作为展示初始态。
...
- `AiExtractionClient`、OpenAI-compatible 适配层、结构化 DTO、校验器和候选物化已接通手动抽取接口；真实模型调用、关系类型业务白名单和复杂实体合并仍需继续评估。
- 已验证当前账号和端点能够调用 `gpt-5.4-mini` 并返回 Prompt 约束的结构化输出；仍未验证原生 JSON Schema 模式、Responses API 结构化抽取或 `text-embedding-3-small`。
- 新增的 `prd-extraction-v2` / `extraction-v2` 分片摘要以及实体 0～160 字符摘要已通过 Fake AI、结构校验和 SQLite/MockMvc 集成测试，但尚未对真实 `gpt-5.4-mini` 验证实体长度遵循、摘要质量和不同分片的重复表达；实体摘要是生成性展示内容，不能替代逐字证据和人工审核。
...
- 流式进度、结构化结果和历史结果已经合并到来源资料弹窗的 AI Tab，并有无运行空态、历史结果读取失败、最近失败回退历史成功和关闭后重新读取逻辑；本次会话缺少浏览器控制接口，未执行这项新增界面的可见浏览器视觉回归、移动端布局或读屏验收。
...
- 来源资料卡片已完成竖直五段布局、整卡片键盘选择、右上角 icon-only 删除入口和指纹迁移；当前会话使用用户截图完成视觉调整，但没有可调用的浏览器控制工具，尚未执行自动化多选、批量按钮、指纹悬浮、移动端布局和读屏回归。
...
- 当前 `document_summary` 仍将多个分片摘要去重后以中文分号直接拼接并截断；它不是自然的全文摘要，可能出现“本分片”措辞、完整句后机械追加分号和跨章节语义断裂。全文摘要二阶段汇总方案已记录在 19.1.3 第 8 项，当前尚未实现、未新增模型调用或数据表字段。
...
新会话开始时，先阅读本节和 `docs/roadmap.md`，然后从 19.1.3 的第五项继续完成真实浏览器与流式交互验证；第 4 项卡片与批量操作代码已完成当前自动验证，不重新搭建前后端脚手架，也不重做文本型 PDF、分页、SQLite 连接池、SSE 流式抽取和三 Tab 来源资料弹窗链路。
...
### 19.1.3 最高优先级：连接池、流式抽取、来源资料交互与审核质量

以下事项是 AI 抽取链路进入可用状态前的配套任务；第 1、2、3、4、6 项和候选审核持久化已完成当前代码与自动验证边界，第 5 项只完成后端/Fake AI 与部分前端状态骨架，后续应先完成第 5 项真实交互验证，再推进第 7 项审核质量切片：
...
3. **已完成当前代码切片：把 AI 输出并入来源资料查看弹窗**：已将“渲染预览/原文预览”扩展为“渲染预览/原文预览/AI 输出”三个 Tab；AI Tab 展示实时文本、分片进度、候选实体/关系/证据/冲突和运行元数据。没有运行记录时显示明确空态；关闭已完成结果后再次打开按 `extractionRunId` 恢复服务端结果；最近失败但有历史成功结果时保留失败提示并展示历史结果；历史读取错误与抽取失败分离；卡片按钮状态统一为“AI 提取”→“提取中…”→“重新提取”，失败为“重试提取”。已通过前端 `npm run typecheck` 和 `npm run build`，本次没有浏览器控制工具，视觉/移动端/读屏仍未验收。
4. **已完成当前自动验证：收窄来源资料卡片、批量操作并简化删除入口**：桌面端三列竖直卡片、中等宽度两列、移动端单列，按“格式与状态 / 名称 / 完整摘要 / 大小与时间 / 按钮”分区，摘要区伸缩以保证元信息与按钮对齐；删除按钮移至右上角 icon-only，包含 `aria-label`、`title` 和现有二次确认。整张卡片可点击或使用 Enter/空格多选，并有卡片级高亮；批量删除由服务端 `deletion-batches` 在事务中执行，批量提取由 `extraction-batches` 交给默认 2 并发有界线程池，每份资料保持独立运行和状态恢复。SHA-256 从卡片迁移到渲染预览 Tab，短值悬浮展示完整值。Java 21 根 Reactor 47 项测试、前端类型检查和生产构建通过；实际多选、批量操作、悬浮、移动端和读屏视觉回归仍待第 5 项补齐。
5. **继续补齐流式和交互状态验证**：后端/Fake AI 已覆盖多分片增量、结构化结果失败、部分 JSON 不落完整结果、失败运行恢复、单连接池并发边界和历史完整结果恢复；本轮前端已补充 AI Tab 空输出、历史结果恢复、最近失败回退、历史读取错误和重试入口。仍未完成真实浏览器视觉回归、连接中断、取消、断线重连、事件续传和真实模型流式兼容性/生产网络代理联调。
...
7. **后续审核质量切片：关系类型白名单与跨分片实体合并评估**：第 4、5 项收口并完成对应交互验证后，再为关系类型建立允许集合和主体/客体类型约束，拒绝模型输出的未知或语义不匹配关系；再使用固定多分片资料评估同一实体在不同分片中的规范化、来源合并和关系去重。两项都必须保留失败样例，不能因为 JSON 合法或证据可反查就直接确认业务语义正确。

8. **已完成方案、待实施：以二阶段模型汇总生成自然全文摘要**：现有分片 `summary` 仅作为内部 Map 阶段素材，不得再通过 `String.join("；", ...)` 直接显示。每份文档完成所有分片抽取后（包括只有一个分片时），新增一次全文汇总模型调用：输入文档名称、业务类型、按原文顺序排列的章节路径和已校验分片摘要；输出一段 1～160 字符的自然中文全文摘要。Prompt 必须禁止“本分片/当前分片”、YAML、候选标识、机械分号拼接和原文外事实，要求从整份文档视角成段表达并以完整句结束。全文摘要是展示信息而非关系证据；需要单独记录全文摘要 Prompt 版本、生成状态和失败原因。若全文汇总失败，已完成的实体、关系、证据与候选图谱事实仍保持成功，`document_summary` 为空并回退导入原文 excerpt，不能回退为拼接摘要或把整次抽取伪装为失败。SSE 应显示“正在生成全文摘要”阶段；自动测试需覆盖单/多分片自然汇总、不等于分号拼接、长度上限、汇总失败不影响候选落库及真实模型质量边界。该方案会额外增加每份文档一次聊天模型调用，不新增 Embedding、RAG 检索或 Agent 编排。
...
1. **已接入当前基础链路**：前端优先加载后端图谱查询结果；空知识空间才保留虚构演示图谱作为初始展示。
2. 实现孤立、失效来源、缺字段和冲突检查。
3. 在文本型 PDF 首版完成后增加 DOCX 解析，并继续实现增量导入和 Markdown/JSON/PNG 导出；扫描 PDF OCR 仍不进入当前首版。
4. **专项方案待评审**：先评审 [`docs/prd/document-tag-and-association-rag-prd.md`](./document-tag-and-association-rag-prd.md)，确认标签审核、五种文档关系白名单、文档关系图主视图、手工关系和“先非向量、后 Embedding”的实施顺序；评审通过后再设计文档标签表、文档-标签关系表、文档关系表和对应接口。
5. **专项方案实施顺序**：先完成固定正负样本和标签领域，再实现无 Embedding 的显式引用/共同标签/关键词候选关联，最后用独立实验评估是否引入向量召回和混合 RAG；未达到评估门槛前不把向量相似度当作正式关系依据。
