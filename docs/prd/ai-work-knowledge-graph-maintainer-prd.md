...
- 当前来源资料旧版分页、悬浮提示、空间切换、导入回第一页、页末删除、历史结果恢复和 `processing` 自动收敛已通过隔离自动化浏览器回归；本轮无限滚动只完成静态构建验证，尚未把触底追加、重复触发抑制和加载更多失败重试纳入浏览器回归，也未把临时脚本纳入仓库级持续回归。
...
新会话开始时，先阅读本节和 `docs/roadmap.md`，然后从 19.1.3 的第五项继续完成浏览器与流式交互验证；优先覆盖无空间创建弹窗、来源资料无限滚动和第 4 项卡片与批量操作，不重新搭建前后端脚手架，也不重做文本型 PDF、分页、SQLite 连接池、SSE 流式抽取和三 Tab 来源资料弹窗链路。
