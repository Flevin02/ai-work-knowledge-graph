...
- 已使用 Java 21 执行根 Reactor `mvn test`，共 36 项测试通过；除分页和 AI 既有回归外，新增覆盖多页 PDF 页码边界、原文预览、原始字节去重、批次部分失败、空间隔离，以及损坏、受密码保护、零页和无文本 PDF；真实模型和真实 Embedding 调用未纳入默认自动测试。
...
新会话开始时，先阅读本节和 `docs/roadmap.md`，然后从 19.1.3 的第一项继续，不重新搭建前后端脚手架，也不重做文本型 PDF 和分页链路。
