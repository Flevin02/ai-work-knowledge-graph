...
新会话开始时，先阅读本节、`docs/roadmap.md` 和 [`docs/prd/document-tag-and-association-rag-prd.md`](./document-tag-and-association-rag-prd.md)，然后按以下顺序推进：先完成 19.1.3 第五项的浏览器与流式交互验证，再进行关系类型白名单与跨分片实体合并评估；二阶段自然全文摘要的 Fake/SQLite/MockMvc 验收已完成，下一步只需补充真实模型质量边界。之后执行专项阶段 0，准备固定正负样本并冻结文档关系图、节点详情跳转、悬浮高亮和默认内容关联规则；再按“无 Embedding 文档内容关联 → 可选标签 → 文档关系图切换 → Embedding/混合 RAG → 固定 RAG 有据问答 → 条件满足后 Agent”推进。当前不提前引入标签、文档关系、会话引用、Embedding 或 Agent 生产代码，也不重新搭建已完成的前后端脚手架、PDF、分页、SQLite 连接池、SSE 流式抽取和三 Tab 来源资料链路。
