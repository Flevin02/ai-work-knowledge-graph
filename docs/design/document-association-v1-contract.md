...
| 双共同标签正负例补充资料 | document-association-tag-threshold-eval-v2 | 阶段 2 |
...
`document-candidate-recall-v3` 只在 `v2` 标签-only 候选上继续增加“共同 confirmed 标签数量至少为 2”的阈值；仍不改变默认 `v1` 内容路径、关系判断、逐字证据校验或人工审核。基础 v1 资料证明它能过滤单标签噪声；独立 v2 补充资料进一步证明它能补充一个内容漏召回正例，也会召回一个共享通用标签的跨项目硬负例。因此数量阈值只作为最低候选门槛保留，不能解释为同项目过滤或正式关系规则。
