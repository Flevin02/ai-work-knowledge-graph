...
状态：专项阶段 0 已冻结，供阶段 1 Fake AI 和服务端实现使用。
...
| 候选召回策略 | document-candidate-recall-v1 | 阶段 1 |
...
## 3. 候选召回策略 document-candidate-recall-v1

第一版不使用 Embedding，也不要求标签已经生成。候选召回顺序固定为：
...
阶段 1 的实际 Prompt 资源必须完整表达以下约束；改变其中任何业务语义都必须升级 Prompt 版本：
...
8. matchedTagIds 只能引用服务端提供且允许参与本次运行的标签；阶段 1 固定返回空数组。
